...
import { PisStyleListRes } from "../../redux/common/commonSlice";

interface Props {
  isShowOurUserPopup?: boolean;
  setIsShowOurUserPopup?: Dispatch<SetStateAction<boolean>>;
  onClose: () => void;
  onOurUserSelect?: (rowIndex: number, item: PagingUserListRes) => void;
...
const UserPopupComponent = memo(
  ({
    ...
    }, [isShowOurUserPopup, nmDept, cdDept]);
    ...
    const onUserRowDoubleClick = (row: PagingUserListRes) => {
      ...
      if (onOurUserSelect && typeof currentAttendRowIndex === "number") {
        onOurUserSelect(currentAttendRowIndex, row);
      }
    };
    ...
    return (
      <Modal show={isShowOurUserPopup} onHide={onClose} size="lg" centered>
        ...
        <div className="modal-table-container gx-3 px-2">
          <PisTable
            ...
            onRowDoubleClick={(row) => {
              ...
              // 2) 실제 선택 핸들러 호출
              onUserRowDoubleClick(row.original);
            }}
            onClose={onClose}
