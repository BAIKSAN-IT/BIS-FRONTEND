...
const Pagination = ({ tableProps, sizePerPageList }: PaginationProps) => {
   ...
   */
  const [pageCount, setPageCount] = useState<number>(Math.max(1, tableProps.pageCount)); // 최소 1페이지
  const [pageIndex, setPageIndex] = useState<number>(tableProps.state.pageIndex);

  useEffect(() => {
    setPageCount(Math.max(1, tableProps.pageCount)); // pageCount가 0이어도 최소 1로 설정( 설정안하면은 페이지가없으면은 1이 안나옴 )
