...
import SearchNeedleAndHangTag from "./SearchNeedleAndHangTag";
import CommonTable from "@components/table/CommonTable";
import useInputRefs from "@utils/useInputRefs";
import {formatDateToYYYYMMDD} from "@utils/CommonUtil";
import {NeedleAndHangTagTableColumns} from "@pages/mainfactory/needle/NeedleAndHangTagTableColumns";
import {NEEDLE_COLUMNS_TYPE} from "@constants/factory/folding/needle";
import {getNeedleActual} from "@redux/factory/factoryNeedleSlice";

import Swal from "sweetalert2";

const NeedleAndHangTag = memo(() => {
  ...
  const [needleList, setNeedleList] = useState<NEEDLE_COLUMNS_TYPE[]>([]);

  /** 화면에 실제로 보여줄 데이터 */
  const [visibleList, setVisibleList] = useState<NEEDLE_COLUMNS_TYPE[]>([]);
  ...
  const columns = useMemo(() => NeedleAndHangTagTableColumns(), []);
  return (
    <>
      <EisPageTitleBar
        pageNm="Factory"
        pageUrl="/NeedleAndHangTag"
        onSearchButtonClick={handleSearch}
        breadCrumbItems={[
          {label: "Needle&HangTag", path: "/needleHangTag", active: true},
        ]}
      />

      <SearchNeedleAndHangTag
        refs={refs}
        onSearchButtonClick={handleSearch}
...
export default NeedleAndHangTag;
