...
export type MetricType = "QTY" | "AMT";
...
/**
 * 최신 월 라벨용 실제 표시 날짜를 결정
 * 1) refs 선택일자가 최신 월과 같으면 그 날짜 사용
 * 2) 데이터에 8자리 날짜가 있으면 그 중 최신 날짜 사용
 * 3) 없으면 YYYYMM01 fallback
 */
const resolveLatestMonthDisplayDateKey = (
  rows: FactoryDashboardDailyKnittingRes[],
  ...
  const latestRowDateKey =
    rows
      .map((row) => normalizeDateKey((row as any)?.dtsWork))
      .filter(
        (dateKey) =>
          dateKey.length >= 8 && getMonthKey(dateKey) === latestMonthKey
      )
      .sort()
      .slice(-1)[0] || "";
...
const toMonthDateKey = (
  rawDate: string,
  latestMonthKey: string,
  latestMonthDisplayDateKey: string
) => {
  const normalized = normalizeDateKey(rawDate);
  const monthKey = normalized.slice(0, 6);
...
const getPrimaryBizarea = (
  ...
  const filteredRows = rows.filter((row) =>
    isToday ? isTodayRow((row as any)?.sw) : !isTodayRow((row as any)?.sw)
  );
...
/**
 * 가동률 계산
 * 1) rate 필드가 있으면 우선 사용
 * 2) 없으면 keep/use 로 직접 계산
 */
const getOperationRateInfo = (row: any) => {
  const machineKeep = toNum(
    row?.qtyMachineKeep ??
    row?.cntMachineKeep ??
    row?.machineKeep ??
    row?.qtyMcKeep ??
  ...
  const operationRate =
    machineKeep > 0 ? calcRate(machineUse, machineKeep) : explicitRate;

  return {
    machineKeep,
    machineUse,
    operationRate: clamp(operationRate, 0, 100),
  };
};
...
const KnittingBarLineChart = memo(
  ({
     refs,
     rows,
     selectedTab,
    ...
    const primaryBizarea = useMemo(
      () => getPrimaryBizarea(rows, isToday),
      [rows, isToday]
    );

    const chartRows = useMemo<KnittingChartRow[]>(() => {
      ...
      const latestMonthKey = !isToday
        ? filtered
        .map((row) => getMonthKey((row as any)?.dtsWork))
        .filter((v) => v.length === 6)
        .sort()
        .slice(-1)[0] || ""
        : "";

      const latestMonthDisplayDateKey = !isToday
        ? resolveLatestMonthDisplayDateKey(filtered, latestMonthKey, refs)
        : "";
      ...
      const mappedRows = filtered
        .map((row) => {
          ...
          const dtsWork = isToday
            ? toDayDateKey(rawDate)
            : toMonthDateKey(
              rawDate,
              latestMonthKey,
              latestMonthDisplayDateKey
            );

          if (!dtsWork) return null;

          const {machineKeep, machineUse, operationRate} =
            getOperationRateInfo(rowAny);
          ...
          return {
            ...
            prodQty: isToday
              ? toNum(rowAny?.qtyDay)
              : toNum(rowAny?.qtyMonthActual),

            prodAmt: isToday
              ? toNum(rowAny?.amtDay)
              : toNum(rowAny?.amtMonthActual),
        ...
        .filter(Boolean) as KnittingChartRow[];

      const grouped = mappedRows.reduce<Record<string, KnittingChartRow[]>>(
        (acc, row) => {
          if (!acc[row.dtsWork]) {
            acc[row.dtsWork] = [];
          }
      ...
      const aggregated = Object.keys(grouped)
        ...
        .map((dateKey) => {
          ...
          const sumQty = rowsByDate.reduce(
            (sum, row) => sum + toNum(row.prodQty),
            0
          );

          const sumAmt = rowsByDate.reduce(
            (sum, row) => sum + toNum(row.prodAmt),
            0
          );

          const sumKeep = rowsByDate.reduce(
            (sum, row) => sum + toNum(row.machineKeep),
            0
          );

          const sumUse = rowsByDate.reduce(
            (sum, row) => sum + toNum(row.machineUse),
            0
          );
          ...
          const avgRate =
            rowsByDate.length > 0
              ? rowsByDate.reduce(
              (sum, row) => sum + toNum(row.operationRate),
              0
            ) / rowsByDate.length
              : 0;

          const operationRate =
            sumKeep > 0 ? calcRate(sumUse, sumKeep) : avgRate;
    ...
    const customDates = useMemo(() => {
      return Array.from(new Set(chartRows.map((item) => item.dtsWork)))
        .filter((v) => v.length === 8)
        .sort();
    }, [chartRows]);

    const currentBarKey = metric === "QTY" ? "prodQty" : "prodAmt";

    const series = useMemo<ComboSeriesDef[]>(() => {
      return [
        {
          key: currentBarKey,
          label: metric === "QTY" ? "생산량" : "생산금액",
          type: "bar",
          color: metric === "QTY" ? "#4e79a7" : "#f28e2b",
          stack: "prod",
          order: 2,
        },
        {
    ...
    const latestDate =
      customDates.length > 0 ? customDates[customDates.length - 1] : "";
    ...
    return (
      <Row className="align-items-stretch d-flex flex-wrap mt-n2">
        <Col xs={12} className="d-flex flex-column">
          ...
          >
            <label style={controlLabelStyle(metric === "QTY")}>
              <Form.Check
                inline
                type="checkbox"
                id={`knitting-shared-qty-${period}`}
                checked={metric === "QTY"}
                onChange={() => onChangeMetric("QTY")}
                label="생산량"
              />
            </label>
            ...
            <label style={controlLabelStyle(metric === "AMT")}>
              <Form.Check
                inline
                type="checkbox"
                id={`knitting-shared-amt-${period}`}
                checked={metric === "AMT"}
                onChange={() => onChangeMetric("AMT")}
                label="AMT"
              />
            </label>
            ...
            <label style={controlLabelStyle(showLineView)}>
              <Form.Check
                inline
                type="checkbox"
                id={`knitting-shared-line-view-${period}`}
                checked={showLineView}
                onChange={(e) => onChangeShowLineView(e.target.checked)}
                label="LINE VIEW"
              />
            </label>
          </div>
...
export default KnittingBarLineChart;
