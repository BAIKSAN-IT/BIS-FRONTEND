...
const KnittingDepartmentList: React.FC<Props> = memo(
  ({ factories, data, leftItemWidth = 160, leftSubWidth = 140, unitWidth = 90 }) => {
    ...
    const rows03 = Array.isArray(data) ? (data as DailyReportListRes[]) : [];
      ...
      <div className="sdm-wrap">
        <table className="sdm-table">
          ...
          <tbody>
            ...
            <tr>
              {factories.map((f) => {
                const [l1, l2, l3] = getTriple(byNoRow, "05", f.key as FactoryKey);
                return (
                  <React.Fragment key={`lines-val-${f.key}`}>
                    <td className={cellCls(f.isTotal)} colSpan={2}>
                      {val(l1)}
            ...
            <tr>
              {factories.map((f) => {
                const [l1, l2, l3] = getTriple(byNoRow, "06", f.key as FactoryKey);
                return (
                  <React.Fragment key={`lines-val-${f.key}`}>
                    <td className={cellCls(f.isTotal)} colSpan={2}>
                      {val(l1)}
            ...
            <tr>
              {factories.map((f) => {
                const [l1, l2, l3] = getTriple(byNoRow, "07", f.key as FactoryKey);
                return (
                  <React.Fragment key={`lines-val-${f.key}`}>
                    <td className={cellCls(f.isTotal)} colSpan={2}>
                      {val(l1)}
            ...
            <tr>
              {factories.map((f) => {
                const [l1, l2, l3] = getTriple(byNoRow, "08", f.key as FactoryKey);
                return (
                  <React.Fragment key={`lines-val-${f.key}`}>
                    <td className={cellCls(f.isTotal)} colSpan={2}>
                      {val(l1)}
            ...
            <tr>
              {factories.map((f) => {
                const [l1, l2, l3] = getTriple(byNoRow, "09", f.key as FactoryKey);
                return (
                  <React.Fragment key={`lines-val-${f.key}`}>
                    <td className={cellCls(f.isTotal)} colSpan={2}>
                      {val(l1)}
            ...
            <tr>
              {factories.map((f) => {
                const [l1, l2, l3] = getTriple(byNoRow, "10", f.key as FactoryKey);
                return (
                  <React.Fragment key={`lines-val-${f.key}`}>
                    <td className={cellCls(f.isTotal)} colSpan={2}>
                      {val(l1)}
            ...
            <tr>
              {factories.map((f) => {
                const [l1, l2, l3] = getTriple(byNoRow, "11", f.key as FactoryKey);
                return (
                  <React.Fragment key={`lines-val-${f.key}`}>
                    <td className={cellCls(f.isTotal)} colSpan={2}>
                      {val(l1)}
            ...
            <tr>
              {factories.map((f) => {
                const [l1, l2, l3] = getTriple(byNoRow, "12", f.key as FactoryKey);
                return (
                  <React.Fragment key={`lines-val-${f.key}`}>
                    <td className={cellCls(f.isTotal)} colSpan={2}>
                      {val(l1)}
            ...
            <tr>
              {factories.map((f) => {
                const [l1, l2, l3] = getTriple(byNoRow, "13", f.key as FactoryKey);
                return (
                  <React.Fragment key={`lines-val-${f.key}`}>
                    <td className={cellCls(f.isTotal)} colSpan={2}>
                      {val(l1)}
            ...
            <tr>
              {factories.map((f) => {
                const [l1, l2, l3] = getTriple(byNoRow, "15", f.key as FactoryKey);
                return (
                  <React.Fragment key={`lines-val-${f.key}`}>
                    <td className={cellCls(f.isTotal)} colSpan={2}>
                      {val(l1)}
            ...
            <tr>
              {factories.map((f) => {
                const [l1, l2, l3] = getTriple(byNoRow, "16", f.key as FactoryKey);
                return (
                  <React.Fragment key={`lines-val-${f.key}`}>
                    <td className={cellCls(f.isTotal)} colSpan={2}>
                      {val(l1)}
...
export default KnittingDepartmentList;
