...
import {getCuttingActualColor} from "@redux/factory/factoryCuttingSlice";
import {
  CUTTING_STYLE_COLOR_COLUMNS,
  CUTTING_STYLE_COLOR_COLUMNS_TYPE,
} from "@constants/factory/cutting/cuttingActualColor";
...
const CuttingActualColor = forwardRef((props: HEADER_PROPS, ref) => {
  ...
  const [cuttingActualList, setCuttingActualList] = useState<CUTTING_STYLE_COLOR_COLUMNS_TYPE[]>([]);
        ...
        if (!isEmpty(payload.data)) {
          if (val.excel === "Y") {
            const currentDate = getVtnTime(val.selectedDate)
              .replace(/[-:T\s]/g, "")
              .slice(0, 14);
            const fileName = `Cutting_Actual(Color)_${currentDate}`;
      ...
      <Card style={{marginTop: "2px"}}>
        <Row>
          <div className="hps-popup-wrapper">
            <CustomTableGrid
              columns={CUTTING_STYLE_COLOR_COLUMNS}
...
export default CuttingActualColor;
