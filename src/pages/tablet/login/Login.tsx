...
import { isEmpty, setFullscreen } from "../../../utils/CommonUtil";

import { createGlobalStyle } from "styled-components";
import { getFactoryList, setUserEnvInfo } from "../../../redux/tablet/tabletSlice";
import { loginUser, restoreAuthSession, setIsFirstLogin } from "../../../redux/common/authSlice";
import { AREA_SELECT_LIST, LANGUAGE_SELECT_LIST } from "../../../constants/common/factory";
...
const Login: React.FC = () => {
  ...
  /*const redirectUrl =
    location.pathname.includes("/auth/login/tablet")
      ? "/tablet/home"
      : location?.search?.slice(6) || "/";*/
  ...
  const onSubmit = () => {
    ...
    dispatch(loginUser(params)).then((res) => {
      ...
      if (payload.status === 200 && !isEmpty(payload.data)) {
        const userDt = payload.data.user.permission;
  ...
  const [inputName, setInputName] = useState<"userId" | "userPassword">("userId");
  const [keyboardVisible, setKeyboardVisible] = useState(false);

  const handleInputFocus = (name: "userId" | "userPassword") => {
    setInputName(name);
      ...
      <AuthLayout>
        ...
        >
          ...
          <FormInput
            ...
            onFocus={() => handleInputFocus("userPassword")}
            onChange={(e) => handleInputChange(e)}
            containerClass={"mb-3"}
            inputMode="none"
          />
                ...
                return (
                  ...
                  >
                    <input
                      ...
                      onChange={(e) => handleInputChange(e)}
                      checked={userInfo.cdBizarea === item.value}
                    />
