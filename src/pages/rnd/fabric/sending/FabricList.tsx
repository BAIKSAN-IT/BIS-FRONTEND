...
import {addSend, mergeSendFiles} from "@redux/rnd/SendingSlice";
...
import SendingTabList from "./SendingTabList";
...
import RecapTabList from "@pages/rnd/fabric/recap/RecapTabList";
import PisVirtualTable from "@components/table/PisVirtualTable";

export type FabricSendingChildAPI = {
  onSearch?: () => void;
  onExcel?: () => void;
  onPrint?: () => void;
};

const FabricList = forwardRef<FabricSendingChildAPI, {}>((_, ref) => {
  ...
  const actAddTo = (bucket: "sending") => {
    if (bucket !== "sending") return;
    if (!checkedRows.length) return;

    // checkedRows에 qrcode 붙여주기
    const rowsWithQr = checkedRows.map((row) => {
      const firstQr = listAll?.rndArticleQrcodeList?.find((qr) => qr.seqArticle === row.seqArticle);
      return {
        ...row,
        qrcode: firstQr?.qrcode ?? "", // 첫 번째 QRCode만 매칭
      };
    });

    // 1) Redux 저장
    dispatch(addSend(rowsWithQr as any));

    // 2) 파일 Redux merge
    if (listAll?.rndArticleFileList?.length) {
      dispatch(mergeSendFiles(listAll.rndArticleFileList));
    }
