...
const SrsNoticePanel = ({
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) =>
      [row.category, row.title, stripHtml(row.content || "")].some((value) => (value || "").toLowerCase().includes(lowerKeyword))
    );
  }, [keyword, rows]);
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setForm((prev) => ({ ...prev, newFiles: [...prev.newFiles, ...selectedFiles] }));
    event.target.value = "";
  };
  ...
  const markExistingFileDeleted = (seqFile: number) => {
    setForm((prev) => ({ ...prev, deletedSeqFiles: Array.from(new Set([...prev.deletedSeqFiles, seqFile])) }));
  };
    ...
    <section className="srs-notice-page">
      ...
      <div className="srs-board-file-area">
        <div className="srs-board-file-head">
          <div>
            <strong>첨부파일</strong>
            <p>{fileLoading ? "첨부파일을 불러오는 중입니다." : "파일을 추가/삭제할 수 있습니다."}</p>
          </div>
        ...
        <input ref={fileInputRef} type="file" multiple className="srs-hidden-file-input" onChange={handleFileChange} />

        <div className="srs-board-file-list">
          {visibleEditFiles.map((file) => (
            <div className="srs-board-file-row" key={`${file.noBoard}-${file.seqFile}`}>
              <span>{displayFileName(file.nmFile)}</span>
              <div>
                <button type="button" onClick={() => onPreviewFile(file.nmFile)}>
                ...
                <button type="button" className="danger" onClick={() => markExistingFileDeleted(file.seqFile)}>
                  삭제
                </button>
              </div>
            </div>
          ...
          {form.newFiles.map((file, index) => (
            <div className="srs-board-file-row pending" key={`${file.name}-${index}`}>
              <span>{file.name}</span>
              <div>
                <em>저장 대기</em>
                <button type="button" className="danger" onClick={() => removeNewFile(index)}>
                  제거
                </button>
              </div>
            </div>
  ...
  return (
    <section className="srs-notice-panel" aria-label="SRS 공지사항">
      ...
      {viewMode === "edit" ? renderEdit() : null}
    </section>
  );

  function setFormValue<K extends keyof SrsNoticeSubmitValue>(key: K, value: SrsNoticeSubmitValue[K]) {
...
const toDateInput = (value: string) => (value && value.length >= 8 ? `${value.slice(0, 4)}-${value.slice(4, 6)}-${value.slice(6, 8)}` : "");
