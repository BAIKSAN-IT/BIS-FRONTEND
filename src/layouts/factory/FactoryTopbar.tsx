...
/* components */
import FactoryDropDownComponent from "@components/FactoryDropdown/FactoryDropDownComponent";
import BizareaDropDownComponent from "@components/BizAreaDropdown/BizareaDropDownComponent";
...
const FactoryTopbar: React.FC<FactoryTopbarProps> = ({btnHandler, titleName, isLoading, lnInfo, isQc, type}) => {
  const navigate = useNavigate();
  const dispatch = useDispatch<AppDispatch>();
  const location = useLocation();

  const {userEnvInfo, initUserEnvInfo, line, factoryLineList} = useSelector((state: RootState) => ({
  ...
  const getInitialSeconds = (): number => {
    ...
    const rule = rules.find(
      (r) =>
        r.processGbn === params.get("processGbn") &&
        r.pathnames.includes(pathname)
    );
    ...
    const defaultMap: Record<string, number> = {
      "/factory/knitting/machine": 60,
    };

    return defaultMap[pathname] ?? reloadTime;
  };
  ...
  //달력 숨김여부 체크.
  const datePickerHidePaths = ["/factory/machine/sewing/management", "/factory/machine/other"];

  //시간초 숨김여부 체크.
  const secHidePaths = ["/factory/machine/sewing/management", "/factory/machine/other"];

  //법인 및 공장 숨김여부 체크.
  const showBizareaAndFtyPaths = [
    "/factory/sewing/hps/list",
    "/factory/sewing/hps/shopFloor",
  ];
  ...
  //법인 및 공장 숨김여부 체크 ( 아이디 권한별 화면 표출 다르게함.)
  useEffect(() => {
    if(initUserEnvInfo.cdBizarea && initUserEnvInfo.cdFty && initUserEnvInfo.cdFtyAll){
      setIsShowBizareaVisible(false); // select box visable
      setIsShowFactoryVisible(false); // select box visable
    }else if(initUserEnvInfo.cdBizarea){
      setIsShowBizareaVisible(false); // select box visable
      setIsShowFactoryVisible(true); // select box visable
    }else{
      setIsShowBizareaVisible(true); // select box visable
      setIsShowFactoryVisible(true); // select box visable
    }
  }, []);
  // 버튼 클릭 핸들러
  ...
  const goHome = () => {
    ...
    // 2) 선택된 Bizarea 기준으로 FactoryList 다시 로드
    dispatch(
      getFactoryList({
        cdCompany: userEnvInfo.cdCompany,
        cdBizarea: userEnvInfo.cdBizarea,
  ...
  const showDatePicker = () => {
    // 숨김 경로에 있으면 아무 동작 안 함
    if (datePickerHidePaths.includes(location.pathname)) {
      return;
    }
  ...
  return (
    <TopLayer>
      ...
      {showBizareaAndFtyPaths.includes(location.pathname) && (
        <SelectArea
          style={{
            display: "flex",
            alignItems: "center",
        ...
        >
          ...
          {isShowBizareaVisible && (
            <div style={{ textAlign: "center" }}>
              <BizareaDropDownComponent top={3} iconSize={30} />
              <p
                style={{
                  width: "100%",
                  marginTop: "-5px",
          ...
          {isShowFactoryVisible && (
            <div style={{ textAlign: "center" }}>
              <FactoryDropDownComponent top={3} iconSize={30} showIcon />
              <p
                style={{
                  width: "100%",
                  marginTop: "-5px",
          ...
          {/* 단일 아이디일 경우 (bago1, pktt1, vina1) 등*/}
          {!isShowBizareaVisible && !isShowFactoryVisible && (
            <div style={{ textAlign: "center" }}>
              <p
                style={{
                  width: "100%",
      ...
      {!secHidePaths.includes(location.pathname) && (
        <SelectArea>
          <span style={{color: "lightsalmon"}}>{String(seconds).padStart(2, "0")} SEC</span>
        </SelectArea>
      )}
      <TitleArea onClick={handleButtonClick}>
